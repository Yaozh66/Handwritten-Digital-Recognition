function [x,fval,exitFlag,output,population,scores] = ga_sa(fun,nvars,Aineq,bineq,Aeq,beq,lb,ub,nonlcon,intcon,options,ann_data,ann_label,layer)
...
if ~isempty(intcon)   
    ...
    switch (output.problemtype)
        ...
        case {'boundconstraints', 'linearconstraints'}
            [x,fval,exitFlag,output,population,scores] = galincon_sa(FitnessFcn,nvars, ...
