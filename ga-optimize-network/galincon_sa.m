function [x,fval,exitFlag,output,population,scores] = galincon_sa(FitnessFcn,GenomeLength, ...
    Aineq,bineq,Aeq,beq,lb,ub,options,output,Iterate,ann_data,ann_label,layer)
...
if  options.Verbosity > 1
    fprintf('\n                                  Best           Mean      Stall\n');
    fprintf('Generation      Func-count        f(x)           f(x)    Generations\n');
end
...
while isempty(exitFlag)
    ...
    for pop = 1:length(totalPop)
        ...
        [score,population,state] = stepGASA(score,population,options,state,GenomeLength,FitnessFcn,ann_data,ann_label,layer);
