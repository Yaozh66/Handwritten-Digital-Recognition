...
if NotVectorized
    for k = 1:maxeval
        ...
        if feasible % All the constraints are satisfied
            [sites(k).f, count, cache] = funevaluate(ObjFunc, Xin, sites(k).x, Cache, cache, cachetol, cachelimit, varargin{:});
...
elseif strcmpi(Completepoll,'on')  % vectorized is 'on', Completepoll MUST be 'on' too
    ...
    [f, count, cache] = funevaluate(ObjFunc, Xin, Xfeas, Cache, cache, cachetol, cachelimit, varargin{:});
