...
if options.NotVectorizedPoll
    if strcmpi(options.CompletePoll,'on')
        ...
        if options.SerialUserFcn
            for k = 1:maxeval
                [trialF(k), count, cache] = funevaluate(ObjFunc,Xin,trialX(:,k),Cache,cache, ...
                    CacheTol,CacheSize,objFcnArg{:});
            end
            nextIterate.cache = cache;
        else
            parfor (k = 1:maxeval)
        ...
        for k = 1:maxeval
            [sites(k).f,count,cache] = funevaluate(ObjFunc,Xin,sites(k).x,Cache,cache, ...
                CacheTol,CacheSize, objFcnArg{:});
...
elseif strcmpi(options.CompletePoll,'on')
    ...
    [f,count,cache] = funevaluate(ObjFunc,Xin,Xfeas,options.Cache,cache,options.CacheTol, ...
