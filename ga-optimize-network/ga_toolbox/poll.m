function  [msg,nextIterate,optimState] = poll(ObjFunc,Xin,Iterate, ...
    MeshSize,Aineq,bineq,Aeq,beq,lb,ub,problemtype,objFcnArg,optimState,options)
...
elseif any(strcmpi(problemtype, {'linearconstraints','nonlinearconstr'}))  % Constraints
  if any(strcmpi(options.PollMethod,{'gsspositivebasisnp1','gsspositivebasis2n'}))
    [DirVector,TangentCone,AugmentedDirs] = gssdirections(AdaptiveMesh,MeshSize,Iterate.x, ...
      Aineq,bineq,lb,ub,optimState.NullBasisAeq,tol);
      if ~isempty(AugmentedDirs)
         AugmentedDirs(:,(~any(AugmentedDirs))) = [];
      end
  else
...
% Remove null directions if any from the core directions.
...
sites = struct('x',cell(numberOfXtrials,1),'f',cell(numberOfXtrials,1));
...
if (msg)
  ...
  if AdaptiveMesh
    ...
    % Find an iterate with lower objective
    [~,nextIterate,~,optimState.FunEval] =  ...
      psnextfeasible(ObjFunc,Xin,site,Iterate,Aineq,bineq,Aeq,beq,lb,ub, ...
                     tol,constr,objFcnArg,optimState.FunEval,options);
  end
