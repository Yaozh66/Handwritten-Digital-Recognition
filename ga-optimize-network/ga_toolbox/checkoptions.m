...
% psoptimset does not support string values for PlotFcn and SearchFcn. As
% such, convert any PlotFcns and SearchFcn which are names of shipping
% functions to function handles before calling psoptimset. Note also we
% have to check if these fields exist because checkoptions can be called
% with options generated by optimset.
if isfield(options, 'PlotFcns')
    options.PlotFcns = replaceEnumStringWithFcnHdl(...
        'PatternsearchOptions', 'PlotFcn', options.PlotFcns);
end
if isfield(options, 'SearchMethod')
    options.SearchMethod = replaceEnumStringWithFcnHdl(...
        'PatternsearchOptions', 'SearchFcn', options.SearchMethod);
end
...
% If UseParallel is true, set UseCompletePoll to true
...
% Validate UseParallel option (logicals or specific strings)
