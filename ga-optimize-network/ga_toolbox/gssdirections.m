function [Basis,TangentCone,AugmentedDirs] = gssdirections(AdaptiveMesh,MeshSize,XCurrent,Aineq,bineq,lb,ub,AeqNull,epsBall)
...
if ~isempty(AeqNull)
    NullBasisAeq = AeqNull;
else
    NullBasisAeq = eye(nVars);
end
    
if AdaptiveMesh
    % Create random generating directions
    pollParam = 1/sqrt(MeshSize);
    lowerT = tril((round((pollParam+1)*rand(nVars)-0.5)),-1);
    diagtemp = pollParam*sign(rand(nVars,1)-0.5);
    diagtemp(~diagtemp) = pollParam*sign(0.5-rand);
    diagT  = diag(diagtemp);
    UncontrBasis = lowerT + diagT;
    order = randperm(nVars);
    if ~isempty(AeqNull)
      NullBasisAeq = UncontrBasis(order,order) - AeqNull;
    else
      NullBasisAeq = UncontrBasis(order,order);
    end
end
